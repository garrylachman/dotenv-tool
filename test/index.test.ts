...
`;
  ...
  assert.deepEqual(tool.contents[0], {
    type: "data",
    lineNumber: 1,
    multiline: false,
    data: { key: "MY_KEY", value: "my-value" },
  });
  assert.deepEqual(tool.contents[1], {
    type: "comment",
    lineNumber: 2,
    value: "# this is a comment",
  });
  assert.deepEqual(tool.contents[2], { type: "blank", lineNumber: 3 });
  assert.deepEqual(tool.contents[3], {
    type: "data",
    lineNumber: 4,
    multiline: false,
...
test("hasKey", async (t) => {
  ...
  assert.strictEqual(tool.getKey("SAMPLE_NUMBER"), 123);
});

test("casting type", async (t) => {
  const tool = new DotEnvTool({
    path: join(__dirname, "fixtures", "sample.env"),
  });
  await tool.load();

  assert.equal(typeof tool.getKey("SAMPLE_NUMBER"), "number");
  assert.equal(typeof tool.getKey("AFTER_A_BLANK_LINE"), "boolean");
  assert.equal(typeof tool.getKey("FOO"), "string");
});
