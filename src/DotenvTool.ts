...
type DataLineValue = string | number | null | boolean;
...
type NewLine =
  | Omit<CommentLine, "lineNumber">
  | Omit<DataLine, "lineNumber">
  | Omit<BlankLine, "lineNumber">;
...
export default class DotEnvTool {
  ...
  private async parse(): Promise<void> {
    ...
    lines.forEach((line: string) => {
      ...
      if (isParsingMultiline) {
      ...
      } else if (line.match(/^[a-zA-Z_]+[a-zA-Z0-9_]*/) != null) {
        // starts with a variable name, it's data
        ...
        if (value === '"""') {
          ...
          newLine = {
            ...
            multiline: false,
            lineNumber: this.nextLine,
            data: { key, value: this.deserializeValue(value) },
          };
        }
  ...
  private serializeValue(line: DataLineValue): string {
    if (line === null) {
      return "";
    }
    return String(line);
  ...
  private deserializeValue(line: string): DataLineValue {
    if (line === "") {
      return null;
    }
    if (line === "true") {
  ...
  addKey(key: string, value: DataLineValue, multiline?: boolean): void {
    this.contents.push({
      type: "data",
  ...
  updateKey(key: string, newValue: DataLineValue): void {
    const dataLine = this.findKey(key);
  ...
  updateLine(lineNumber: number, newLine: NewLine): void {
    if (lineNumber > this.contents.length) {
      throw new Error(
        `Trying to update line ${lineNumber}, but the file has only ${this.contents.length} lines.`
      );
    }
        ...
        case "data":
          if (line.data.value !== null) {
            const value = line.multiline
              ? `"""\n${line.data.value}\n"""`
              : this.serializeValue(line.data.value);
